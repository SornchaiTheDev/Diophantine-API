...
const find = ({ p, ans, xn = [], origin, final = [] }) => {
  if (p.length <= 2) {
    final.push(
      ...twoFormular(ans, origin.slice(0, 2)).map((result) => result.concat(xn))
    );

    return final;
  }
  nFormular(ans, p.slice(0, -1), p).map((result) => {
    const result_ans = ans - result * p[p.length - 1];
    const mo_xn = xn.concat(result).reverse();
    find({
      p: p.slice(0, -1),
      ans: result_ans,
      xn: mo_xn,
...
console.log(find({ p: p, ans: ans, origin: p }));
